...
float scene(vec3 p) {
    // vec3 fbmCoord = (p + 2.0 * vec3(u_time, 0.0, u_time)) / 1.5;
    // float d = sdSphere(p- vec3(-3.0, 1.0 + 0.5 * sin(u_time), -1.0), 1.5);
    // d = smin(d, sdSphere(p - vec3(1.0 * cos(u_time), 0.0, 0.0), 2.0), 2.0);
    // d = smin(d, sdSphere(p - vec3(1.0, 0.0, -0.2 * cos(u_time)), 2.0), 0.7) + 1.0 * fbm(fbmCoord / 3.0);
    // d = smin(d, p.y + 2.5, 7.0);
...
float scene2(vec3 p) {
    return fbm(p + 2.0 * vec3(u_time, u_time, u_time) / 6.0) - p.y;
}
...
vec3 raymarch(vec3 rayOrigin, vec3 rayDirection) {    
    ...
    for (int i = 0; i < MAX_STEPS; i++) {
        ...
        float density = scene2(p);

        if (density > 1e-3) {
          vec4 color = vec4(mix(vec3(0.0), vec3(0.75), density * 0.8), density);
          color.w *= 0.8;
          color.rgb *= color.w;
...
void main() {
    ...
    float sun = clamp( dot(lightPosition,rd), 0.0, 0.95);

    // background sky
    vec3 color = vec3(0.76,0.75,0.86);
	color += 0.2*vec3(1.00,0.60,0.10)*pow( sun, 8.0 );
