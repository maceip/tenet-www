...
float scene(vec3 p) {
        float displacement = cnoise(p.yy + u_time * 0.5) / 4.0;
    float d = sdSphere(p, 0.75);
    float d2 = sdSphere(p - vec3(cos(u_time), sin(u_time), 0.0), 0.25);
    float d3 = sdSphere(p - vec3(cos(u_time), cos(u_time), 0.0), 0.25);
