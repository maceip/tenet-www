...
vec3 getColor(float amount) {
    vec3 color = 0.5 + 0.5 * cos(6.2831 * (vec3(0.2, 0.0, 0.67) + amount * vec3(0.9, 0.5, 0.5)));
    return color * amount;
}
...
float scene(vec3 p) {
    // p.xyz = p.xzy;
    vec3 p1 = p;//rotate(p, vec3(1.0, 1.0, 1.0), u_time * 0.3);
    ...
    for(int i = 0; i<5; i++) {
        r = sqrt(dot(z,z));
        if(r > 2.0) break;
        theta = atan(z.y / z.x) + u_time * 0.2;
        phi = acos(z.z / r) + u_time * 0.5;
...
void main() {
    ...
    vec3 ro = vec3(uCamera.x, uCamera.y, uCamera.z);

     vec3 lookAt = vec3(0, 0, 0);
    // Ray Direction
    vec3 rd = getCam(ro, lookAt) * normalize(vec3(uv, 1.0));
    vec3 lightPosition = vec3(4.0 + cos(-u_time * 0.5 ) * 4.0, 4.0 + sin(u_time * 0.5) * 4.0, -4.0 + cos(u_time * 0.5) * 5.0);
   ...
   vec3 p = ro + rd * d;
    ...
    if(d<MAX_DIST) {
        ...
        float specular = pow(clamp(dot(reflect(lightDirection, normal), rd), 0.0, 1.0), 10.0);
        float shadow = softShadows(p, lightDirection, 0.1, 5.0, 64.0);
        vec3 lin = vec3(1.0) * (diffuse + specular) * shadow; 
        vec3 poweredDistanceBasedColors = getColor(pow(d, 1.65)) + 0.65;
        color = lin  * poweredDistanceBasedColors;
    }

    color = fog(color , d, ro, rd, lightDirection);
